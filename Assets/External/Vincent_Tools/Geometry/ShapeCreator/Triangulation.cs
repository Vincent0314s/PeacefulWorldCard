...
namespace PolygonTool
{
    ...
    public class Triangulation
    {
        ...
        public static int[] PolygonToTriangles(List<Vector3> polygonVertex, bool filpTriangle = true)
        {
            ...
            triangulation.SetCompareAxle(CompareAxle.X);
        ...
        public static Mesh GenPolyMesh(List<Vector3> polyVerts, bool filpNormal = true)
        {
            var resMesh = new Mesh();
            var triangles = PolygonToTriangles(polyVerts, filpNormal);
            resMesh.vertices = polyVerts.ToArray();
            resMesh.triangles = triangles;
            resMesh.RecalculateNormals();
            resMesh.RecalculateBounds();
            return resMesh;
        }

        public static Mesh GenPolyMesh(Vector3[] _verticies, int[] _triangles, Vector2[] _uv)
        {
            var resMesh = new Mesh();
            resMesh.vertices = _verticies;
            resMesh.triangles = _triangles;
            resMesh.uv = _uv;
